...
def it(image_name):
    ...
    image = cv2.cvtColor(cv2.imread(f"C:\\Users\\dimen\\point-classifier-clear\\data\\images\\{image_name}.jpg"),
                         cv2.COLOR_BGR2RGB)
    colorless_point_cloud = o3d.io.read_point_cloud("C:\\Users\\dimen\\point-classifier-clear\\data\\point-clouds\\lamp_post.ply")
    ...
    for point in colorless_point_cloud.points:
        projected_point = project_on_image(point,
                                           np.array([cx, cy, cz]),
                                           np.array([roll, pitch, heading]))
